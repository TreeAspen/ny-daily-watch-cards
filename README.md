...
The export takes its **frame rate from the footage** — the longest clip sets it, a still-only reel stays at 30 — so 24, 25, 30 and 60fps sources all map one source frame to one output frame instead of being resampled onto a grid that does not divide evenly. Each clip's own rate is measured as it comes in, and every seek lands in the middle of a source frame rather than on its edge. Measured on constant-rate test clips, repeated frames went from 17.5 / 15 / 30.8 / 0 per cent (24 / 25 / 30 / 60fps) to 2.1 / 1 / 0 / 0.8.
...
导出的**帧率跟随素材**——由最长的那段片段决定，纯静态图仍是 30fps——所以 24 / 25 / 30 / 60fps 的片源都是一帧对一帧，而不是重采样到一个除不尽的网格上。每段片段进来时会实测自己的帧率，取帧时也落在源帧的正中而不是边界上。用固定帧率的测试片实测，重复帧从 17.5 / 15 / 30.8 / 0 %（24 / 25 / 30 / 60fps）降到 2.1 / 1 / 0 / 0.8 %。
