/* Vendored from Anthropic's data-visualisation skill (scripts/validate_palette.js).

   Kept verbatim so the studio checks a custom palette with exactly the same
   math the brand palette was chosen with: OKLCH lightness band, chroma floor,
   OKLab CVD separation under the Machado-Oliveira-Fernandes simulation, the
   normal-vision floor, and WCAG contrast against the chart surface.

   It only auto-runs when <body> carries data-palette, so importing validate()
